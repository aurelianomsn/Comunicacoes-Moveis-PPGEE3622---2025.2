...
sinais_tx = cell(length(deltas), 1);
tempos = cell(length(deltas), 1);
...
% Exibir os três sinais transmitidos em subplots
figure;
for k = 1:length(deltas)
    subplot(3,1,k);
    plot(tempos{k}/deltas(k), real(sinais_tx{k}), 'b', 'LineWidth', 1.5);
    grid on;
    xlabel('Tempo / \delta t');
    ylabel('Amplitude');
    title(['Sinal Transmitido - \delta t = ', num2str(deltas(k)), ' s']);
end

% Loop completo de multipercurso
for k = 1:length(deltas)
    delta_t = deltas(k);
