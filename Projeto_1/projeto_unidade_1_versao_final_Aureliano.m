...
ganho_canal = sum(pot_preliminar(2:end));  % Potência total dispersa (sem a componente LoS)

% Normalização das componentes dispersas
pot_dispersa_normalizada = (1/(Kr+1)) * (pot_preliminar / ganho_canal);

% Ajuste da componente LoS (apenas o caminho de menor atraso)
pot_dispersa_normalizada(1) = Kr / (Kr + 1);

% Verificação de ganho total unitário
ganho_total = sum(pot_dispersa_normalizada);
...
% Converter radianos para graus e restringir entre 0° e 90°
angulos_elevacao_final_deg = rad2deg(angulos_elevacao_finais);
angulos_elevacao_final_deg = max(min(angulos_elevacao_final_deg, 90), 0);  % limita entre 0° e 90°
...
% Criar gráfico polar (considerando apenas 0–90°)
subplot(1,2,2)
...
r_mat = reshape(r_n, [3, qtd_blocos]);
...
desvio_doppler_1(1)=0;
desvio_doppler_1(34)=0;
figure;
subplot(1,2,1)
...
desvio_doppler_2(1)=0;
desvio_doppler_2(34)=0;
subplot(1,2,2)
...
for k = 1:length(deltas)
    delta_t = deltas(k);
    [t, s_tx] = gerar_pulso(0, delta_t, Nt);  % pulso transmitido
    ...
    for n = 1:length(componente_multipercurso_ordenado)
        ...
        %  pulso atrasado
        pulso_atrasado = min(round(tau_n / dt), Nt);  % nunca maior que Nt
...
ganho_canal = sum(pot_dispersa_normalizada);

% Parâmetros de varredura
kappa_Hz = logspace(0, 10, 501);  % 1 Hz até 10^10 Hz
kappa_MHz = kappa_Hz / 1e6;       % converter para MHz

sigma_s = logspace(-3, 1, 501);   % 1 ms até 10 s aproximadamente
...
% Autocorrelação em frequência ρ_TT(κ; 0)
rho_k = arrayfun(@(k) rho_TT(k, 0, pot_dispersa_normalizada, ...
    componente_multipercurso_ordenado, desvio_doppler_1, ganho_canal), kappa_Hz);

rho_k_mag = abs(rho_k);

figure;
semilogx(kappa_Hz, rho_k_mag, 'b', 'LineWidth', 1.5); hold on;
xlabel('\kappa [Hz]');
...
banda_coerencia_Hz = zeros(size(thresholds));
...
for ii = 1:length(thresholds)
    ...
    if isempty(cross_idx)
        ...
        Tc_Hz = kappa_Hz(cross_idx);
    end
    banda_coerencia_Hz(ii) = Tc_Hz;

    xline(Tc_Hz, '--', 'Color', [0 0.45 0.9], ...
        'Label', sprintf('\\rho_B=%.2f → %.3e Hz', th, Tc_Hz), ...
...
% Autocorrelação temporal ρ_TT(0; σ)
v_list = [5, 50];
nu_list = {desvio_doppler_1, desvio_doppler_2};
tempo_coerencia_s = zeros(length(v_list), length(thresholds));
...
for v_idx = 1:length(v_list)
    ...
    % Vetor sigma em segundos (linha base para log)
    if vrx == 5
        sigma_log = logspace(-3, 1, 501);  
    elseif vrx == 50
        sigma_log = logspace(-4, -1, 501); 
    end

    % Autocorrelação temporal
    rho_s = arrayfun(@(s) rho_TT(0, s, pot_dispersa_normalizada, ...
        componente_multipercurso_ordenado, nu, ganho_canal), sigma_log);
    rho_s_mag = abs(rho_s);

    subplot(length(v_list), 1, v_idx);
    semilogx(sigma_log, rho_s_mag, 'LineWidth', 1.5); hold on;
    xlabel('\sigma [s]');
    ylabel('| \rho_{TT}(0,\sigma) |');
    title(sprintf('Autocorrelação temporal ρ_{TT}(0,σ) (v_{rx} = %d m/s)', vrx));
    grid on;

    % Marcar tempos de coerência
    for tt = 1:length(thresholds)
        ...
        if isempty(cross_idx)
            ...
            Tc_s = sigma_log(cross_idx);
        end
        tempo_coerencia_s(v_idx, tt) = Tc_s;

        xline(Tc_s, '--', ...
            sprintf('\\rho_T=%.2f → %.3e s', th, Tc_s), ...
            'Color', [0 0.6 0.9], 'LabelVerticalAlignment', 'bottom', ...
            'LabelOrientation', 'aligned');
    end

    % Limites de X logarítmicos
    xlim([sigma_log(1) sigma_log(end)]);
    ylim([0 1.05]);
    set(gca, 'XScale', 'log', 'XMinorGrid', 'on');
end
